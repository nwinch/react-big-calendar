...
var Selection = function () {
  ...
  Selection.prototype._touchStart = function _touchStart(e) {
    ...
    if (!this.globalMouse && node && !(0, _contains2.default)(node, e.target)) {
      ...
      var top = _normalizeDistance2.top;
      var left = _normalizeDistance2.left;
      var bottom = _normalizeDistance2.bottom;
      var right = _normalizeDistance2.right;
  ...
  Selection.prototype._touchEnd = function _touchEnd(e) {
    ...
    console.log('contains', (0, _contains2.default)(this.container(), e.target));

    var inRoot = (0, _contains2.default)(this.container(), e.target);
    var click = this.isTouch(e.changedTouches[0].pageX, e.changedTouches[0].pageY);

    this._touchStartData = null;

    console.log('_touchEnd reset', click, !inRoot);

    if (click && !inRoot) {
      return this.emit('reset');
    }

    if (click && inRoot) return this.emit('click', { x: e.changedTouches[0].pageX, y: e.changedTouches[0].pageY });
  ...
  Selection.prototype.isTouch = function isTouch(pageX, pageY) {
    var _touchStartData = this._touchStartData;
    var x = _touchStartData.x;
    var y = _touchStartData.y;
